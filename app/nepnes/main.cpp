...
#include <vendor/noc/noc_file_dialog.h>
...
int main(int, char **)
{
  ...
  GLFWwindow *window = glfwCreateWindow(1280, 720, "nepnes", NULL, NULL);
  ...
  while (!glfwWindowShouldClose(window))
  {
    ...
    // Render the UI, obtain UI interactions.
    render_ui(ui);

    if (ui.show_open_file_dialog)
    {
      if (const char *path{noc_file_dialog_open(NOC_FILE_DIALOG_OPEN, nullptr, nullptr, nullptr)};
          path)
      {
        printf("Picked: %s\n", path);
      }
      ui.show_open_file_dialog = false;
    }
