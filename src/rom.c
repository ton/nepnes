...
int write_rom_information(FILE *fp, uint8_t *rom_data)
{
  ...
  fprintf(fp, string_fmt, left_width, "Cartridge contains battery backed PRG RAM:",
          header.has_battery_backed_vram ? "Yes" : "No");

  fprintf(fp, string_fmt, left_width, "Mirroring:", mirroring_to_string(header.mirroring));
  fprintf(fp, string_fmt, left_width, "Console type:", console_type_to_string(header.console_type));
  fprintf(fp, "; %-*s %s (%d)\n", left_width, "Mapper:", mapper_to_string(header.mapper),
          header.mapper);
